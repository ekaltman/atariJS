...
function SimulatorWidget(button, mem) {
  ...
  function Simulator() {
    ...
    function reset() {
      //display.reset();
      for (var i = 0; i < 0xf000; i++) { // clear ZP, stack and screen
        memory.set(i, 0x00);
      }
      regA = regX = regY = 0;
      regPC = 0xf000;
  ...
  function Assembler() {
    ...
    function checkBranch(param, opcode) {
      ...
      pushByte(opcode);
      if (addr < (defaultCodePC - 0xf000)) {  // Backwards?
        pushByte((0xff - ((defaultCodePC - 0xf000) - addr)) & 0xff);
        return true;
      }
      pushByte((addr - (defaultCodePC - 0xf000) - 1) & 0xff);
      return true;
    }
    ...
    function hexdump() {
      //openPopup(memory.format(0xf000, codeLen), 'Hexdump');
      $("#hexDump").text(memory.format(0xf000, codeLen));
    }
